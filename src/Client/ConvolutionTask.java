...
public class ConvolutionTask implements Task {
    ...
    public ConvolutionTask(int id, byte[] serializedImage,int [] kernel) {
        this.id = id;
        this.serializedImage = serializedImage;
        //ConvolutionTask.kernelInBytes =kernelInBytes;
        this.kernel=kernel;
        //System.out.println("from constructor:"+ConvolutionTask.kernel + ", size is "+ ConvolutionTask.kernel.length);
        //showArray(ConvolutionTask.kernel);
    }


    public BufferedImage convertByteArrayToBufferedImage(byte[] imageBytes) {
    ...
    public byte[] convertBufferedImageToByteArray(BufferedImage image, String format) {
    ...
    public byte[] splitImage(byte[] serializedImage, int square) {
    ...
    public byte[] mergeImages(byte[] topLeftSerialized, byte[]  topRightSerialized, byte[]  bottomLeftSerialized, byte[]  bottomRightSerialized) throws IOException {
    ...
    public void sendToSlave(byte [] serializedImage, Socket socket) throws IOException {
        OutputStream outputStream = socket.getOutputStream();
        DataOutputStream dataOutputStream = new DataOutputStream(outputStream);

//        for (Float aFloat : ConvolutionTask.kernelInFloat) {
...
//        }
    ...
    public TaskResult execute() {
        try {
            Socket socketTopRight = new Socket("localhost",4030);
            Socket socketTopLeft = new Socket("localhost",4040);
            Socket socketBottomLeft = new Socket("localhost",4050);
            Socket socketBottomRight = new Socket("localhost",4060);
